...
def _build_parser() -> argparse.ArgumentParser:
    ...
    companies_parser = subparsers.add_parser(
        "companies-build",
        help=(
            "Fetch S&P 500 company names and GICS sectors into the companies "
            "table (current membership; not point-in-time)."
        ),
    )
...
def main() -> None:
    ...
    if args.command == "metrics-build":
        LOG.info("Running metrics build: warehouse_path=%s", args.warehouse_path)
        result = build_metrics_trend(warehouse_path=args.warehouse_path)
    ...
    if args.command == "companies-build":
        ...
        result = build_companies(warehouse_path=args.warehouse_path)
        LOG.info("Companies build completed: %s", result)
        for key, value in result.items():
            print(f"{key}={value}")
        return 0
