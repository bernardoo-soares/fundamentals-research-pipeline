...
from trading_bot.steps.sec_submissions import run_sec_submissions_ingestion


class _FakeSubmissionsClient:
    def __init__(self, fail_cik: str | None = None):
    ...
    def fetch_submissions(self, cik: str) -> dict:
        ...
        if self.fail_cik and cik == self.fail_cik:
...
def test_run_sec_submissions_ingestion_writes_json_and_log(tmp_path) -> None:
    mapping_path = tmp_path / "sec_cik_mapping.csv"
    raw_dir = tmp_path / "raw" / "sec" / "submissions"
    log_path = tmp_path / "reports" / "sec_submissions_ingestion_log.csv"
...
def test_run_sec_submissions_ingestion_logs_error_rows(tmp_path) -> None:
    mapping_path = tmp_path / "sec_cik_mapping.csv"
    raw_dir = tmp_path / "raw" / "sec" / "submissions"
    log_path = tmp_path / "reports" / "sec_submissions_ingestion_log.csv"
    ...
    client = _FakeSubmissionsClient(fail_cik="0001111111")
    df = run_sec_submissions_ingestion(
        mapping_path=mapping_path,
        raw_dir=raw_dir,
        log_path=log_path,
