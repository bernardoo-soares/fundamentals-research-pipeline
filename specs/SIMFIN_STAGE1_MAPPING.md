...
| `dvpq` | `unsupported` | no preferred-dividend field | no preferred-dividend field | no preferred-dividend field | `dvpq` is **preferred** dividends (Compustat: "Dividends - Preferred/Preference"). SimFin publishes no such column, so this is null in the SimFin era. It was previously mapped to `Dividends Paid` (the **total**), which made `dividend_payer_years_10y` read ~0 for genuine payers before 2023. Totals now live in `dvy`. |
...
| `dvy` | `direct` with annual dataset and sign transform | annual `Dividends Paid` | same | same | Total cash dividends, year-to-date basis. Legacy counterpart is Compustat `dvy` ("Cash Dividends"). Equivalence measured 2026-07-24 on 274 FY2023 overlap tickers: 92.9% agree within 1%, median relative difference 0.0000 (AAPL 15025, KO 7952, MSFT 19800 match exactly). |
